...
//socket
socket.on("create", (g) => { setPlayArea(g); });
socket.on("restart", (g) => { setPlayArea(g); });
socket.on("show", (x, y) => { show(x, y); });
socket.on("mark", (x, y) => { setTargetState(x, y, 'mark') });
socket.on("hover", (x, y, isOver) => { setTargetState(x, y, isOver ? 'hover' : 'unHover') });
...
function targetExists(x, y) {
    ...
    if (!mainBoard)
        return false;
    else if (!mainBoard.childNodes[x])
        return false;
    else if (!mainBoard.childNodes[x].childNodes[y])
        return false;
    else
        return true;
}
...
function setTargetState(x, y, state) {
    ...
    switch (state) {
        ...
        case 'show':
            ...
            break;
        case 'mark':
            if (!target.className.includes("_mark") && target.className.includes("_hidden")) {
...
function showHidden(target) {
    target.className = target.className.replace("_hidden", "").replace("_hover", "");
}
...
function showEndScreen(name, success) {
    ...
    if (success)
        ...
        text = insults[parseInt(Math.random() * insults.length)].replace('{x}', name);
